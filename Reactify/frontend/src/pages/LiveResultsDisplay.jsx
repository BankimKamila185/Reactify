...
export const LiveResultsDisplay = () => {
    ...
    // Color palette for options - matching Reactify
    const optionColors = [
        ...
        const initSession = async () => {
            try {
                ...
                if (response.success) {
                    ...
                    // Initialize participant count from API response
    ...
    const handlePrevPoll = () => {
        if (currentPollIndex > 0) {
            ...
            // Socket broadcast for other clients (also updates DB via socketHandlers)
    ...
    // Render Bar Chart - Reactify style with growing bars
    const renderBarChart = () => (
        <div className="reacti-bar-chart">
            {results.map((option, index) => (
                <motion.div
                    key={option.id || index}
                    className="reacti-bar-item"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                ...
                >
                    ...
                    {/* Growing bar */}
                    <div className="reacti-bar-wrapper">
                        <motion.div
                            className="reacti-bar"
                    ...
                    <div
                        className="reacti-bar-line"
                        style={{ backgroundColor: option.color }}
                    />

                    {/* Option label */}
                    <div className="reacti-option-label">{option.text || `Option ${index + 1}`}</div>
                </motion.div>
            ))}
        </div>
    );
    ...
    const renderPollContent = () => {
        ...
        if (hideResults) {
            return (
                <div className="reacti-results-hidden">
                    <span>👁</span>
    ...
    if (loading) {
        return (
            <div className="reacti-presentation loading">
                <div className="reacti-loader"></div>
    ...
    return (
        <div className="reacti-presentation">
            {/* Top Black Bar */}
            <div className="reacti-top-bar">
                <button className="reacti-close-btn" onClick={() => navigate(`/poll/edit/${session?.presentationId || sessionId}`)}>
                    ×
                </button>

                <div className="reacti-join-pill">
                    <span>Join at <strong>reactify.com/join</strong> | use code</span>
                    <strong className="reacti-code">{formatJoinCode(sessionCode)}</strong>
                </div>

                <div className="reacti-logo">
                    ...
                    <span>Reactify</span>
                    <button className="reacti-fullscreen-btn">⛶</button>
                </div>
            </div>

            {/* White Content Area */}
            <div className="reacti-content">
                {/* Side Tools - Left margin */}
                <div className="reacti-side-tools">
                    <button className="reacti-tool-btn" title="Pointer">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                            <path d="M3 2l9 6-4 1-2 4-3-11z" fill="currentColor" />
                        </svg>
                    </button>
                    <button
                        className="reacti-tool-btn"
                        title="Timer"
                    ...
                    <button
                        className={`reacti-tool-btn ${showPercent ? 'active' : ''}`}
                        title="Percentage"
                        onClick={() => setShowPercent(!showPercent)}
                    >
                        %
                    </button>
                    <button className="reacti-tool-btn" title="Chart">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                ...
                <div className="reacti-main-content">
                    {/* Question */}
                    <motion.h1
                        className="reacti-question"
                        key={currentPoll?._id}
                    ...
                    <div className="reacti-visualization">
                        {renderPollContent()}
                    </div>
                </div>

                {/* QR Code - Right side */}
                <div className="reacti-qr-section">
                    <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(`${window.location.origin}/vote/${sessionId}`)}`}
                        alt="QR Code to join"
                        className="reacti-qr-code"
                    />
                </div>
            </div>

            {/* Bottom Black Bar - Always Visible like Reactify */}
            <div className="reacti-bottom-bar visible">
                {/* Left Section - Navigation */}
                <div className="reacti-nav-left">
                    <button
                        className="reacti-nav-btn"
                    ...
                    <button className="reacti-pointer-btn" title="Pointer">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    ...
                    <button
                        ...
                        onClick={currentPollIndex >= polls.length - 1 ? () => navigate(`/poll/edit/${sessionId}`) : handleNextPoll}
                        disabled={false}
                ...
                {/* Center Section - Controls (Reactify style) */}
                <div className="reacti-controls-center">
                    {/* Fullscreen */}
                    <button className="reacti-control-btn" title="Fullscreen">
                    ...
                    {/* Comments */}
                    <button
                        className={`reacti-control-btn ${showFeedback ? 'active' : ''}`}
                    ...
                    {/* Q&A */}
                    <button className="reacti-control-btn" title="Q&A">
                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                    ...
                    {/* Grid */}
                    <button className="reacti-control-btn" title="Grid">
                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                    ...
                    <button
                        className={`reacti-control-btn ${hideResults ? 'active' : ''}`}
                    ...
                    {/* Lock */}
                    <button
                        className={`reacti-control-btn ${isLocked ? 'active' : ''}`}
                ...
                {/* Right Section - Info */}
                <div className="reacti-nav-right">
                    {countdown && (
                        <div className="reacti-countdown-display">
                            {formatCountdown(countdown)}
                        </div>
                    )}
                    <div className="reacti-participant-count" title="Participants">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    ...
                    <div className="reacti-slide-counter">
            ...
            <div
                className="reacti-hover-trigger"
            ...
            <AnimatePresence>
                {showJoinOverlay && (
                    <motion.div
                        className="reacti-join-overlay"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        onClick={() => setShowJoinOverlay(false)}
                    >
                        <div className="reacti-join-content" onClick={e => e.stopPropagation()}>
                            <button className="reacti-overlay-close" onClick={() => setShowJoinOverlay(false)}>×</button>
                            <h2>Go to reactify.com/vote</h2>
                            <p>and use the session ID</p>
                            <div className="reacti-big-code">{sessionId}</div>
                            <div className="reacti-qr-placeholder">
                                <img
                                    src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(`${window.location.origin}/vote/${sessionId}`)}`}
                                    alt="QR Code"
                                />
            ...
            <AnimatePresence>
                {showCountdownModal && (
                    <motion.div
                        className="reacti-modal-overlay"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    >
                        <motion.div
                            className="reacti-countdown-modal"
                        ...
                        >
                            <h3>Set countdown</h3>
                            <div className="reacti-countdown-options">
                            ...
                            <button className="reacti-modal-cancel" onClick={() => setShowCountdownModal(false)}>
                                Cancel
                            </button>
                        </motion.div>
                    </motion.div>
